...
include $(JSR120_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += wma11
endif

ifeq ($(ENABLE_JSR_75), true)
include $(JSR75_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += jsr75
endif

ifeq ($(ENABLE_JAMS), true)
include $(JAMS_DIR)/makefiles/classpath.make
endif

ifeq ($(ENABLE_DIO), true)
include $(DIO_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += josh-dio
endif

ifeq ($(ENABLE_CELLULAR), true)
include $(CELLULAR_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += josh-cellular
endif

ifeq ($(ENABLE_EXTRA_PROTOCOLS), true)
include $(EXTRA_PROTOCOLS_DIR)/makefiles/classpath.make
endif

ifeq ($(ENABLE_DIRECTUI), true)
include $(DIRECTUI_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += josh-directui
endif

ifeq ($(ENABLE_MEDIA), true)
include $(MEDIA_DIR)/makefiles/classpath.make
...
ifeq ($(ENABLE_SENSORS), true)
include $(SENSORS_DIR)/makefiles/classpath.make
endif

include $(LOGGING_UTIL_DIR)/makefiles/classpath.make

ifeq ($(ENABLE_SECURITY), true)
include $(SECURITY_DIR)/makefiles/classpath.make
JOSH_BUILD_MODULES += josh-crypto
endif

ifeq ($(ENABLE_JSR280), true)
include $(JSR280_DIR)/makefiles/classpath.make
...
#----------------------------------------------------------------------
#
# Build documents and packages for SDK
#
#----------------------------------------------------------------------

.PHONY: doc_all package_all doc_clean package_clean

SDK_LIB_DIR=$(SDK_DIR)/lib
SDK_BIN_DIR=$(SDK_DIR)/bin
SDK_TMPLIB_DIR=$(SDK_LIB_DIR)/tmpclasses

CLDC_BUILD_CLASSES_PATH := $(JVMWorkSpace)/build/classes
...
.SECONDEXPANSION:
EXT_JAR_packages=$(foreach mod,$(JOSH_BUILD_MODULES), $(SDK_LIB_DIR)/$(mod).zip)
CLDC_JAR_packages=$(SDK_LIB_DIR)/cldc111.zip
...
package_clean:
	rm -f $(SDK_LIB_DIR)/*.jar
	rm -rf $(SDK_TMPLIB_DIR)

$(JOSHVM_SIMU_EXECUTABLE) : $(OUTDIR)/dist/bin/cldc_vm.exe
	cp $< $@
	
$(PREVERIFY_EXECUTABLE) : $(BUILD_ROOT_DIR)/preverifier/build/win32/preverify.exe
	cp $< $@
	
$(SDK_LIB_DIR)/%.zip : FORCE
	$(foreach package,$(subst .,/,$(DOC_$(subst .zip,,$(@F))_PACKAGES)), mkdir -p $(SDK_TMPLIB_DIR)/$(package);cp -P --force $(CLDC_BUILD_CLASSES_PATH)/$(package)/* $(SDK_TMPLIB_DIR)/$(package);echo Generating $(DOC_$(subst .zip,,$(@F))_PACKAGES) ...;)
	$(JDK_DIR)/bin/jar -cfm `cygpath -p -w "$@"` \
				`cygpath -p -w $(SDK_LIB_DIR)/manifests/$(subst .zip,,$(@F))/MANIFEST.MF` \
				$(foreach package,$(subst .,/,$(DOC_$(subst .zip,,$(@F))_PACKAGES)), -C `cygpath -p -w $(SDK_TMPLIB_DIR)` $(package))
	rm -rf $(SDK_TMPLIB_DIR)/$(package)

DOC_SOURCE_cldc111_PATH=$(BUILD_ROOT_DIR)/cldc/src/javaapi/cldc1.1.1
DOC_SOURCE_PATH := $(DOC_SOURCE_PATH)$(BUILD_ROOT_DIR)/cldc/src/javaapi/share$(DOC_PATH_SEP)$(DOC_SOURCE_PATH)$(DOC_SOURCE_cldc111_PATH)$(DOC_PATH_SEP)

DOC_PACKAGES = $(foreach mod,$(JOSH_BUILD_MODULES), $(DOC_$(mod)_PACKAGES))
DOC_cldc111_PACKAGES = java.io java.lang java.lang.ref java.util java.security
DOC_PACKAGES += $(DOC_cldc111_PACKAGES) 

DOC_gcf_PACKAGES = javax.microedition.io
DOC_PACKAGES += $(DOC_gcf_PACKAGES)
