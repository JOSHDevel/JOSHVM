...
ROMGEN_CFG_LST = $(GEN_DIR)/rom_configs.lst

ifeq ($(ENABLE_JSR_120), true)
include $(JSR120_DIR)/makefiles/vm_module.make
endif

ifeq ($(ENABLE_JSR_75), true)
include $(JSR75_DIR)/makefiles/vm_module.make
endif

ifeq ($(ENABLE_JAMS), true)
include $(JAMS_DIR)/makefiles/vm_module.make
endif

ifeq ($(ENABLE_DIO), true)
include $(DIO_DIR)/makefiles/vm_module.make
endif

ifeq ($(ENABLE_CELLULAR), true)
include $(CELLULAR_DIR)/makefiles/vm_module.make
endif

ifeq ($(ENABLE_EXTRA_PROTOCOLS), true)
include $(EXTRA_PROTOCOLS_DIR)/makefiles/vm_module.make
endif
...
ifeq ($(ENABLE_JAVACALL_TEST), true)
ENABLE_CFLAGS += -DENABLE_JAVACALL_NATIVE_TEST=1
endif

ifeq ($(ENABLE_JAMS), true)
ENABLE_CFLAGS += -DENABLE_STARTUP_RUN_GREENLANDJAR=0
else
...
ifeq ($(ENABLE_JSR_120), true)
Obj_Files           +=         $(JSR120_Obj_Files)
ENABLE_CFLAGS       +=         -DENABLE_JSR_120
endif

ifeq ($(ENABLE_JSR_75), true)
Obj_Files           +=         $(JSR75_Obj_Files)
ENABLE_CFLAGS       +=         -DENABLE_JSR_75
endif

ifeq ($(ENABLE_JAMS), true)
Obj_Files           +=         $(JAMS_Obj_Files)
endif

ifeq ($(ENABLE_DIO), true)
Obj_Files           +=         $(DIO_Obj_Files)
ENABLE_CFLAGS       +=         -DENABLE_DIO
endif

ifeq ($(ENABLE_CELLULAR), true)
Obj_Files           +=         $(CELLULAR_Obj_Files)
ENABLE_CFLAGS       +=         -DENABLE_CELLULAR
endif
...
ifeq ($(ENABLE_SECURITY), true)
Obj_Files           +=         $(SECURITY_Obj_Files)
ENABLE_CFLAGS       +=         -DENABLE_SECURITY
ifeq ($(ENABLE_SECURITY_NATIVE_RSA_SIGNATURE), true)
ENABLE_CFLAGS       +=         -DENABLE_SECURITY_NATIVE_RSA_SIGNATURE
endif
endif

Obj_Files           +=         $(LOGGING_Obj_Files)
...
$(CLDC_ZIP): $(JAVA_FILES)
	...
	$(A)files="$?"; \
	 ...
	 echo "$$files" > $(BuildSpace)/tmpjavasrcfilelist; \
	 $(JAVAC) -g$(JAVAC_DEBUG) -d $(TMPCLASSES) \
                  -bootclasspath $(CLASSES) \
                  -classpath $(CLASSES) @$(BuildSpace)/tmpjavasrcfilelist
...
CPP_INCLUDE_DIRS += $(JSR_EXT_INCLUDE_DIRS)
...
$(LOOP_GENERATOR): $(BUILD_PCH) $(Obj_Files) \
	...
	$(A)$(LINK) $(PCSL_LIBS) $(LINK_FLAGS) /out:$@ $(Obj_Files) $(BUILD_PCH)\
		   InterpreterSkeleton.obj OopMapsSkeleton.obj
...
$(ROM_GENERATOR): $(BUILD_PCH) $(CLDC_ZIP) $(Obj_Files) OopMapsSkeleton.obj $(GP_TABLE_OBJ)
	$(A)echo "linking ROM generator: $(ROM_GENERATOR)"
	$(LINK) $(PCSL_LIBS) $(LINK_FLAGS) /out:$@ \
		$(Obj_Files) $(BUILD_PCH) OopMapsSkeleton.obj $(GP_TABLE_OBJ)
	$(A)$(ROM_GENERATOR) +GenerateOopMaps
	$(A)$(MAKE) OopMaps.obj
	$(A)echo "re-linking ROM generator: $(ROM_GENERATOR)"
	$(LINK) $(PCSL_LIBS) $(LINK_FLAGS) /out:$@ $(Obj_Files) $(BUILD_PCH) \
                OopMaps.obj \
                $(GP_TABLE_OBJ)
	$(A)$(VC_MANIFEST_EMBED_EXE)
	$(A)echo generated `pwd`/$@
...
$(ROM_GENERATOR): $(BUILD_PCH) $(Obj_Files) InterpreterSkeleton.obj \
	...
	$(A)$(LINK) $(PCSL_LIBS) $(LINK_FLAGS) /out:$@ \
		$(Obj_Files) $(BUILD_PCH) Interpreter_$(arch).obj OopMaps.obj
	$(A)$(VC_MANIFEST_EMBED_EXE)
	$(A)echo generated `pwd`/$@
...
ifneq ($(ENABLE_C_INTERPRETER), true)
Interpreter_$(arch)$(OBJ_SUFFIX): Interpreter_$(arch)$(ASM_SUFFIX)
	$(A) echo "ASMing $<"
	$(A)$(ASM) $(ASM_FLAGS) -o Interpreter_$(arch)$(OBJ_SUFFIX) \
		$(THIS_DIR)/Interpreter_$(arch)$(ASM_SUFFIX)
else
AsmStubs_$(target_arch)$(OBJ_SUFFIX): $(JVMWorkSpace)/src/vm/cpu/c/AsmStubs_$(target_arch)$(ASM_SUFFIX)
	$(A)echo "generating CPU stubs $<"
#	$(A)$(CPREP) -D$(host_os) $< > $(THIS_DIR)/AsmStubs_$(target_arch)$(ASM_SUFFIX)
	cp $< $(THIS_DIR)/AsmStubs_$(target_arch)$(ASM_SUFFIX)
	$(A)$(ASM) $(ASM_FLAGS) -o $@ $(THIS_DIR)/AsmStubs_$(target_arch)$(ASM_SUFFIX)
endif
