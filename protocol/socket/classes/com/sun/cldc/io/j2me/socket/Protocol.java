...
/* #endif */
	...
	public Connection openPrim(String name) throws IOException {
		return openPrim(name, Connector.READ_WRITE, true);
	}
...
/* #endif */
    ...
    */
	...
	protected static native int getHostByName0(byte hostname[]);
    protected static native int open0(int in_addr, int port, int mode);
...
/* #endif */
    ...
    synchronized public int read(byte b[], int off, int len)
        ...
        int count = Protocol.readBuf(parent.handle, b, off, len);
        if (count == -1) {
            eof = true;
        }
