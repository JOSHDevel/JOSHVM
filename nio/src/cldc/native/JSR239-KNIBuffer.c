...
Java_java_nio_ByteBufferImpl_finalize() {
	...
	KNI_DeclareHandle(directParentObject);
	
    KNI_GetThisPointer(thisObject);
	KNI_GetObjectClass(thisObject, clazz);

    native_buffer_address = (void*)KNI_GetIntField(thisObject, KNI_GetFieldID(clazz, "arrayOffset", "I"));
	KNI_GetObjectField(thisObject, KNI_GetFieldID(clazz, "directParent", "Ljava/nio/Buffer;"), directParentObject);
	isDirect = KNI_GetBooleanField(thisObject, KNI_GetFieldID(clazz, "isDirect", "Z"));
	
    if (native_buffer_address && isDirect && KNI_IsNullHandle(directParentObject)) {        
