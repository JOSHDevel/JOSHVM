...
public final class Tensor {
  ...
  private void throwIfTypeIsIncompatible(Object o) {
    ...
    if (oType != dtype) {
      ...
      throw new IllegalArgumentException(
          new StringBuffer().append("Cannot convert between a TensorFlowLite tensor with type ")
          					.append(dtype)
          					.append(" and a Java object of type ")
          					.append(o.getClass().getName())
          					.append(" (which is compatible with the TensorFlowLite type ")
          					.append(oType)
          					.append(").").toString());
    }
  }
  ...
  private void throwIfSrcShapeIsIncompatible(Object src) {
    if (isBuffer(src)) {
      ...
      if (bytes != srcBytes) {
        throw new IllegalArgumentException(
        		new StringBuffer().append("Cannot copy to a TensorFlowLite tensor (")
					.append(name())
					.append(") with ")
					.append(bytes)
					.append(" bytes from a Java Buffer with ")
					.append(srcBytes)
    ...
    if (!Arrays.equals(srcShape, shapeCopy)) {
    	throw new IllegalArgumentException(
        		new StringBuffer().append("Cannot copy to a TensorFlowLite tensor (")
					.append(name())
					.append(") with shape ")
					.append(Arrays.toString(shapeCopy))
					.append(" from a Java object with shape ")
					.append(Arrays.toString(srcShape))
					.append(".").toString());
    }
  }
  ...
  private void throwIfDstShapeIsIncompatible(Object dst) {
    if (isBuffer(dst)) {
      ...
      if (bytes > dstBytes) {
        throw new IllegalArgumentException(
            new StringBuffer().append("Cannot copy from a TensorFlowLite tensor (")
            				  .append(name())
            				  .append(") with ")
            				  .append(bytes)
            				  .append("bytes to a Java Buffer with ")
            				  .append(dstBytes)
            				  .append(" bytes.").toString());
      }
      return;
    ...
    if (!Arrays.equals(dstShape, shapeCopy)) {
      throw new IllegalArgumentException(
			  ...
			  .append(") with shape ")
			  .append(Arrays.toString(shapeCopy))
			  .append(" to a Java object with shape ")
			  .append(Arrays.toString(dstShape))
			  .append(".").toString());
    }
  }
