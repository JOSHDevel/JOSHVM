...
final class NativeInterpreterWrapper {
  ...
  public void close() {
    ...
    Enumeration e = ownedDelegates.elements();
    while ( e.hasMoreElements()) {
    	Delegate ownedDelegate = (Delegate)e.nextElement();
      try {
        ownedDelegate.close();
      } catch (Exception ex) {
        System.err.println("Failed to close flex delegate: " + ex);
      }
    }
    ownedDelegates.removeAllElements();
  }

  public void runSignature(
      Hashtable inputs, Hashtable outputs, String methodName) {
    ...
    Hashtable inputsWithInputIndex = new Hashtable();
    Hashtable outputsWithOutputIndex = new Hashtable();
    Enumeration e = inputs.keys();
    while(e.hasMoreElements()) {
    	String key = (String)e.nextElement();
      int tensorIndex =
          getInputTensorIndexFromSignature(interpreterHandle, key, methodName);
      inputsWithInputIndex.put(tensorToInputsIndexes.get(new Integer(tensorIndex)), inputs.get(key));
    }
    e = outputs.keys();
    while(e.hasMoreElements()) {
    	String key = (String)e.nextElement();
      int tensorIndex =
          getOutputTensorIndexFromSignature(interpreterHandle, key, methodName);
      outputsWithOutputIndex.put(tensorToOutputsIndexes.get(new Integer(tensorIndex)), outputs.get(key));
    }
    Object[] inputsList = new Object[inputs.size()];
    int index = 0;
    for (e = inputsWithInputIndex.keys(); e.hasMoreElements(); ) {
    	String key = (String)e.nextElement();
      inputsList[index++] = inputsWithInputIndex.get(key);
    }
    run(inputsList, outputsWithOutputIndex);
  }

  /** Sets inputs, runs model inference and returns outputs. */
  void run(Object[] inputs, Hashtable outputs) {
    ...
    long inferenceStartNanos = System.currentTimeMillis();
    run(interpreterHandle, errorHandle);
    long inferenceDurationNanoseconds = System.currentTimeMillis() - inferenceStartNanos;
    ...
    for (Enumeration e = outputs.keys(); e.hasMoreElements();) {
    	Integer key = (Integer)e.nextElement();
      // Null output placeholders are allowed and ignored.
      if (outputs.get(key) != null) {
        getOutputTensor(key.intValue()).copyTo(outputs.get(key));
      }
    }
  ...
  int getInputIndex(String name) {
    if (inputsIndexes == null) {
      ...
      inputsIndexes = new Hashtable();
      if (names != null) {
        for (int i = 0; i < names.length; ++i) {
          inputsIndexes.put(names[i], new Integer(i));
        }
      }
    }
    if (inputsIndexes.containsKey(name)) {
      return ((Integer)inputsIndexes.get(name)).intValue();
    } else {
      throw new IllegalArgumentException(
    		  new StringBuffer().append("Input error: '")
    		  					.append(name)
    		  					.append("' is not a valid name for any input. Names of inputs and their indexes are ")
    		  					.append(inputsIndexes).toString());
    }
  }
  ...
  private void initTensorIndexesMaps() {
    ...
    tensorToInputsIndexes = new Hashtable();
    tensorToOutputsIndexes = new Hashtable();
    int inputCount = getInputTensorCount();
    for (int i = 0; i < inputCount; ++i) {
      int tensorIndex = getInputTensorIndex(interpreterHandle, i);
      tensorToInputsIndexes.put(new Integer(tensorIndex), new Integer(i));
    }
    int outputCount = getOutputTensorCount();
    for (int i = 0; i < outputCount; ++i) {
      int tensorIndex = getOutputTensorIndex(interpreterHandle, i);
      tensorToOutputsIndexes.put(new Integer(tensorIndex), new Integer(i));
    }
  }
  ...
  int getOutputIndex(String name) {
    if (outputsIndexes == null) {
      ...
      outputsIndexes = new Hashtable();
      if (names != null) {
        for (int i = 0; i < names.length; ++i) {
          outputsIndexes.put(names[i], new Integer(i));
        }
      }
    }
    if (outputsIndexes.containsKey(name)) {
      return ((Integer)outputsIndexes.get(name)).intValue();
    } else {
      throw new IllegalArgumentException(
    		  new StringBuffer().append("Input error: '")
    		  					.append(name)
    		  					.append("' is not a valid name for any output. Names of outputs and their indexes are ")
    		  					.append(outputsIndexes).toString());
    }
  }
  ...
  Long getLastNativeInferenceDurationNanoseconds() {
    return (inferenceDurationNanoseconds < 0) ? null : new Long(inferenceDurationNanoseconds);
  }
  ...
  Tensor getInputTensor(String inputName, String methodName) {
    ...
    initTensorIndexesMaps();
    Integer tensorIndex = new Integer(getInputTensorIndexFromSignature(interpreterHandle, inputName, methodName));
    if (!tensorToInputsIndexes.containsKey(tensorIndex)) {
      throw new IllegalArgumentException(
    		  new StringBuffer(64).append("Invalid input tensor name (")
				.append(inputName)
				.append(") for signature (")
				.append(methodName)
				.append(").").toString());
    }
    return getInputTensor(((Integer)tensorToInputsIndexes.get(tensorIndex)).intValue());
  }
  ...
  Tensor getOutputTensor(String outputName, String methodName) {
    ...
    if (!tensorToOutputsIndexes.containsKey(new Integer(tensorIndex))) {
      throw new IllegalArgumentException(
          new StringBuffer(64).append("Invalid output tensor name (")
          					.append(outputName)
          					.append(") for signature (")
          					.append(methodName)
          					.append(").").toString());
    }
    return getOutputTensor(((Integer)tensorToOutputsIndexes.get(new Integer(tensorIndex))).intValue());
  }
  ...
  private void applyDelegates(Interpreter.Options options) {
    ...
    if (originalGraphHasUnresolvedFlexOp) {
      ...
      if (optionalFlexDelegate != null) {
        ownedDelegates.addElement(optionalFlexDelegate);
    ...
    try {
    	Enumeration e = options.delegates.elements();
      while (e.hasMoreElements()) {
    	Delegate delegate = (Delegate)e.nextElement();
        applyDelegate(interpreterHandle, errorHandle, delegate.getNativeHandle());
        delegates.addElement(delegate);
      }
...
//      }
  ...
  private static Delegate maybeCreateFlexDelegate(Vector delegates) {
    try {
      ...
      Enumeration e = delegates.elements();
      while (e.hasMoreElements()) {
    	  Delegate delegate = (Delegate)e.nextElement();
  ...
  private Hashtable inputsIndexes;
  private Hashtable outputsIndexes;
  // Lazily constructed maps of tensor index to index in input and output indexes.
  private Hashtable tensorToInputsIndexes;
  private Hashtable tensorToOutputsIndexes;
  ...
  private final Vector ownedDelegates = new Vector();
