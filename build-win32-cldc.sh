...
#If ENABLE_SECURITY_NATIVE_RSA_SIGNATURE=true, USE_BOUNCYCASTLE must be true as well in ${SECURITY_DIR}/makefiles/module.config
ENABLE_SECURITY_NATIVE_RSA_SIGNATURE=false
ENABLE_JSR280=true
ENABLE_MEDIA=true
ENABLE_NIO=true
ENABLE_TFLITE=false
...
SDK_DIR=${BUILD_ROOT_DIR}/sdk
JAVADOC_OUTPUT_DIR=${BUILD_ROOT_DIR}/doc
...
then
	...
	if [ "${ERASE_BUILD_OUTPUT}" != "true" ]; then
		...
		make USE_NATIVE_AMS=${USE_NAMS} \
			...
			ENABLE_SECURITY_NATIVE_RSA_SIGNATURE=${ENABLE_SECURITY_NATIVE_RSA_SIGNATURE} \
